...
@systems.register('baseline-neus-system')
class BaselineNeusSystem(BaseSystem):
    """
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ['train']:
            ...
            #NOTE: to get ground truth transients, we sum across the time dimensino, divide by the dataset scale, then gamma correct
            rgba_sum = rgb.sum(-2)
        ...
        if stage in ["train"]:
            batch.update({
                'rays_dict': rays_dict, # (n_rays*rep, 6) or (n_rays*rep + patch_size**2, 6) if using regnerf or (n_rays*rep + patch_size**2 + sparse_rays, 6) if using regnerf and sparse_rays
                'rgb': rgb,
