...
class BaseSystem(pl.LightningModule, SaverMixin):
    ...
    def on_train_batch_start(self, batch, batch_idx, unused=0):
        self.dataset = self.trainer.datamodule.train_dataloader().dataset #this creates a BlenderIterableDataset
        self.preprocess_data(batch, 'train') #updates the batch
        update_module_step(self.model, self.current_epoch, self.global_step) 
    
    def on_validation_batch_start(self, batch, batch_idx, dataloader_idx): #This gets called when validation starts
        self.dataset = self.trainer.datamodule.val_dataloader().dataset #this creates a BlenderDataset
        self.preprocess_data(batch, 'validation')
        # update_module_step(self.model, self.current_epoch, self.global_step)
    
    def on_test_batch_start(self, batch, batch_idx, dataloader_idx):
        self.dataset = self.trainer.datamodule.test_dataloader().dataset #this creates a BlenderDataset
