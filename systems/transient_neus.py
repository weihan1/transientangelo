...
class TransientNeuSSystem(BaseSystem):
        ...
        if self.config.model.use_reg_nerf:     
            if out["num_samples_regnerf"] > 0:
                if self.config.model.train_patch_size>0:
                    ...
                    # self.save_weight_plot(f"it{self.global_step}-weight_regnerf_train_plot", out["weights_patch"], self.model.exposure_time, out["midpoints_patch"],out["depth_patch"], out["ray_indices_patch"], depth_variance=depth_variance_patch)
    ...
    def test_step(self, batch, batch_idx):  
        ...
        np.save(self.get_save_path(f"it{self.global_step}-test/{batch['index'][0].item()}_transient_gt_array"), gt_pixs)
