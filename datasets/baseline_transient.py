...
class BaselineDatasetBase():
    def setup(self, config, split):
        ...
        if self.config.use_gt_depth_normal and self.split == "train": #only need these for training 
            ...
            self.all_normals = compute_normals(self.all_depths.reshape(-1, self.h, self.w), self.K, self.all_c2w) #(N, H, W, 3), not going to apply any normalization or masking here
