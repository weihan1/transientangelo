...
class ProgressiveBandHashGrid(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.register_buffer('current_level', torch.tensor([self.start_level], dtype=torch.long))
        rank_zero_info(f"Grid level is starting at {self.current_level.item()}")
        self.register_buffer("mask", torch.zeros(self.n_level * self.n_features_per_level, dtype=torch.float32, device=get_rank()))

    def forward(self, x):
        enc = self.encoding(x) #(n_samples, n_output_dims)
        enc = enc * self.mask
        return enc

    def update_step(self, epoch, global_step):
        current_level = min(self.start_level + max(global_step - self.start_step, 0) // self.update_steps, self.n_level)
        if current_level > self.current_level.item(): 
            rank_zero_info(f' Update grid level to {current_level} 🍩')
        self.current_level.fill_(current_level)
        
        #Every time the grid level upgrades, you unlock self.n_features_per_level from the encoder
        self.mask[:self.current_level.item() * self.n_features_per_level] = 1.
...
def get_mlp(n_input_dims, n_output_dims, config):
    if config.otype == 'VanillaMLP':
        ...
        with torch.cuda.device(get_rank()):
            #NOTE: use network.params to inspect the weights of the network
