...
class MarchingCubeHelper(nn.Module):
    ...
    def grid_vertices(self):
        if self.verts is None:
            #The resolution controlls the number of vertices sampled
...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface_(self, vmin, vmax):
        def batch_func(x):
            ...
            rv = self.forward_level(x).cpu() #calls forward_level of volume density class
            cleanup()
            return rv
        
        #NOTE: Need to ensure that self.config.isosurface.chunk is a divisor of self.helper.grid_vertices().shape[0]
        #NOTE: self.helper.grid_vertices() depend on resolution
        ...
        if torch.isnan(mesh['v_pos']).any():
            ...
            vertices_to_remove = torch.isnan(mesh["v_pos"]).any(dim=1)
            filtered_vertices = mesh["v_pos"][~vertices_to_remove] #vertices with no NaNs
            
            index_mapping = torch.full((mesh["v_pos"].size(0),), -1, dtype=torch.long) #creates a tensor of -1 with the same size as old vertices
            index_mapping[~vertices_to_remove] = torch.arange(filtered_vertices.size(0)) #creates a mapping from old vertices to new vertices
            
            valid_faces_mask = ~vertices_to_remove[mesh["t_pos_idx"]].any(dim=1) #Checks if any of the vertices in the faces are NaN (shape (n_faces,)
            filtered_faces = mesh["t_pos_idx"][valid_faces_mask] #faces with no NaNs
            updated_faces = index_mapping[filtered_faces] 
            
            mesh["v_pos"] = filtered_vertices
            mesh["t_pos_idx"] = updated_faces
...
class VolumeDensity(BaseImplicitGeometry):
    ...
    def forward(self, points):
        #Verified to be the same as transient NeRF 
        points = contract_to_unisphere(points, self.radius, self.contraction_type) #the same as transient nerf
        ...
        density = density * selector #zero out densities that fall out of selector
...
class VolumeSDF(BaseImplicitGeometry):
    def setup(self):
        ...
        if self.grad_type == 'finite_difference' and isinstance(self.finite_difference_eps, float):
            rank_zero_info(f"Using constant finite difference to compute gradients with eps={self.finite_difference_eps}")
            ...
            with torch.set_grad_enabled(self.training or (with_grad and self.grad_type == 'analytic')):
                ...
                if with_grad:
                    if self.grad_type == 'analytic':
                        ...
                        eps = self._finite_difference_eps.item()
    ...
    def update_step(self, epoch, global_step):
        ...
        if self.grad_type == 'finite_difference':
            ...
            #Logic: if self.finite_difference_eps is a float, use constant finite_diff_eps. Else, update it based on neuralangelo's update equation
            if isinstance(self.finite_difference_eps, float):
                self._finite_difference_eps.fill_(self.finite_difference_eps)
                
                
            elif self.finite_difference_eps == 'progressive':
                #NOTE: progressive finite_difference_eps
                ...
                #NOTE: captured dataset actually has a much smaller grid size, so the finite difference epsilon starts out much smaller 
                grid_size = 2 * self.config.radius / grid_res
                if grid_size != self._finite_difference_eps:
                    rank_zero_info(f"\n Update finite_difference_eps to {grid_size} 🧞")
                    self._finite_difference_eps.fill_(grid_size)
            else:
