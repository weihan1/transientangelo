...
@models.register('baseline-neus-captured')
class NeuSModel(BaseModel):
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.variance, epoch, global_step) #Doesn't update anything

        cos_anneal_end = self.config.get('cos_anneal_end', 0) #max_steps
    ...
    def forward_regnerf(self, unseen_rays):
        ...
        result.update({"depth_patch": depths,
    ...
    def forward_(self, rays):
        ...
        if self.training:
            out.update({
                ...
                'ray_indices': ray_indices.view(-1),
                'distances_from_origin': distances_from_origin.view(-1)             
            })
            if self.config.geometry.grad_type == 'finite_difference':
                out.update({
